    ...
    describe "if statements" do
      describe "if .. then .. end if" do
        ...
        it "does not have an Else node" do
          ...
          node.count{|e| e.is_a?(Parser::Nodes::Else) }.should == 0
        ...
        it "has a 'cond' Expression" do
          ...
          node.find{|e| e.named?(:cond) }.should be_a(Parser::Nodes::Expression)
        end

        it "can have missing body" do
          node = parse(:statement, 'IF cond THEN END IF;')
          node.count{|e| e.instance_of?(Parser::Nodes::Statement) }.should == 1
        end

        it "can have comment body" do
          node = parse(:statement, 'IF cond THEN /* removed */ END IF;')
          node.count{|e| e.instance_of?(Parser::Nodes::Statement) }.should == 1
          node.count{|e| e.instance_of?(Parser::Nodes::TComment) }.should == 1
          node.find{|e| e.instance_of?(Parser::Nodes::TComment) }.source_text.should == '/* removed */'
        end

        it "can have single statement body" do
          node = parse(:statement, 'IF cond THEN a := 10; END IF;')
          node.count{|e| e.instance_of?(Parser::Nodes::Statement) }.should == 2
        end

        it "can have multiple statement body" do
          node = parse(:statement, 'IF cond THEN a := 10; b := 10; END IF;')
          node.count{|e| e.instance_of?(Parser::Nodes::Statement) }.should == 3
        end

        it "can contain comments" do
          node = parse(:statement, "IF cond /* comment */ THEN -- foo\n  NULL; /* foo */ END IF;")
          node.should be_a(Parser::Nodes::Statement)
          node.count{|e| e.is_a?(Parser::Nodes::TComment) }.should == 3
        end
      end
      ...
      describe "if .. then .. else .. end if" do
        ...
        it "has an Else node named 'else'" do
          ...
          node.count{|e| e.named?(:else) and e.is_a?(Parser::Nodes::Else) }.should == 1
        ...
        it "can have multiple statement body" do
          ...
          node.count{|e| e.instance_of?(Parser::Nodes::Statement) }.should == 4
        end
      end
      ...
      describe "if .. then .. elsif .. then .. end if" do
        ...
        it "can have many elsif branches" do
          ...
          node.count{|e| e.is_a?(Parser::Nodes::If) }.should == 6
          node.count{|e| e.is_a?(Parser::Nodes::If) and e.named?(:else) }.should == 5
        end
      ...
      describe "if .. then .. elsif .. then .. else .. endif" do
        ...
        it "has an If node named 'else'" do
          ...
          node.count{|e| e.named?(:else) and e.is_a?(Parser::Nodes::If) }.should == 1
          node.find{|e| e.named?(:else) and e.is_a?(Parser::Nodes::If) }.source_text.should == 'ELSIF cond THEN a := 20; ELSE a := 30; '
        end

        it "has an Else node named 'else'" do
          node = parse(:statement, @text)
          node.count{|e| e.named?(:else) and e.is_a?(Parser::Nodes::Else) }.should == 1
          node.find{|e| e.named?(:else) and e.is_a?(Parser::Nodes::Else) }.source_text.should == 'ELSE a := 30; '
        end

        it "has two If nodes" do
          node = parse(:statement, @text)
          node.count{|e| e.is_a?(Parser::Nodes::If) }.should == 2
        end
      end
    end
